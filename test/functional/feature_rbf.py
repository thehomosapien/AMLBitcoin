...
def make_utxo(node, amount, confirmed=True, scriptPubKey=CScript([1])):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < AmlBit_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, AmlBit_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(AmlBitcoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 AML - so this attempt is rejected.
        dbl_tx = CTransaction()
