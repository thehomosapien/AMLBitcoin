...
#include "config/AmlBitcoin-config.h"
#endif

#include "AmlBitcoingui.h"

#include "AmlBitcoinunits.h"
...
const std::string AmlBitcoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AmlBitcoinGUI::DEFAULT_WALLET = "~Default";

AmlBitcoinGUI::AmlBitcoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void AmlBitcoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a AmlBitcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and AmlBitcoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to AML addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your AmlBitcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified AmlBitcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a AmlBitcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible AmlBitcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void AmlBitcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void AmlBitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** AML START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/AmlBitcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* AmlBitcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        AmlBitcoinLabelLayout->addWidget(label);
        AmlBitcoinLabelLayout->addWidget(toolbar);
        AmlBitcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        AmlBitcoinLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("AmlBitcoin Market Price"));
...
void AmlBitcoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AmlBitcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AmlBitcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AmlBitcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AmlBitcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void AmlBitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AmlBitcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AmlBitcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AmlBitcoinGUI::optionsClicked()
{
...
void AmlBitcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AmlBitcoinGUI::showDebugWindow()
{
...
void AmlBitcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AmlBitcoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AmlBitcoinGUI::openClicked()
{
...
void AmlBitcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AmlBitcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AmlBitcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AmlBitcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AmlBitcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AmlBitcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** AML START */
void AmlBitcoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void AmlBitcoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void AmlBitcoinGUI::gotoManageAssetsPage()
{
...
void AmlBitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to AmlBitcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AmlBitcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AmlBitcoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AmlBitcoinGUI::updateHeadersSyncProgressLabel()
{
...
void AmlBitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AmlBitcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("AmlBitcoin"); // default title
...
void AmlBitcoinGUI::changeEvent(QEvent *e)
{
...
void AmlBitcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AmlBitcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AmlBitcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "AML")
        msg += tr("Amount: %1\n").arg(AmlBitcoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(AmlBitcoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void AmlBitcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AmlBitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AmlBitcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AmlBitcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AmlBitcoinGUI::setHDStatus(int hdEnabled)
{
...
void AmlBitcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AmlBitcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AmlBitcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AmlBitcoinGUI::detectShutdown()
{
...
void AmlBitcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AmlBitcoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void AmlBitcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AmlBitcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void AmlBitcoinGUI::subscribeToCoreSignals()
{
...
void AmlBitcoinGUI::unsubscribeFromCoreSignals()
{
...
void AmlBitcoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<AmlBitcoinUnits::Unit> units = AmlBitcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AmlBitcoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(AmlBitcoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (AmlBitcoinUnits::Unit u : AmlBitcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AmlBitcoinUnits::name(u)), this);
...
{
    setText(AmlBitcoinUnits::name(newUnits));
}
...
void AmlBitcoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=AMLBTC"));
