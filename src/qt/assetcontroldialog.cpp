    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/AmlBitcoinProject/AmlBitcoin/issues/5716
...
{
    GUIUtil::setClipboard(AmlBitcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void AssetControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of AmlBits so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    int nDisplayUnit = AmlBitcoinUnits::AML;
    ...
    l2->setText(AmlBitcoinUnits::formatWithCustomName(QString::fromStdString(strAssetName), nAssetAmount));        // Amount
    l3->setText(AmlBitcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(AmlBitcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(AmlBitcoinUnits::formatWithCustomName(QString::fromStdString(strAssetName), nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    // how many AmlBits the estimated fee can vary per byte we guess wrong
    double dFeeVary = (nBytes != 0) ? (double)nPayFee / nBytes : 0;

    QString toolTip4 = tr("Can vary +/- %1 AmlBit(s) per input.").arg(dFeeVary);
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapAssetCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show AmlBitcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, AmlBitcoinUnits::format(nDisplayUnit, nAmount));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, AmlBitcoinUnits::format(nDisplayUnit, nSum));
