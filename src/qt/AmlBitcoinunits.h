...
#ifndef AMLBITCOIN_QT_AMLBITCOINUNITS_H
#define AMLBITCOIN_QT_AMLBITCOINUNITS_H
...
/** AmlBitcoin unit definitions. Encapsulates parsing and formatting
   and serves as list model for drop-down selection boxes.
*/
class AmlBitcoinUnits: public QAbstractListModel
{
    Q_OBJECT

public:
    explicit AmlBitcoinUnits(QObject *parent);

    /** AmlBitcoin units.
      @note Source: https://en.AmlBitcoin.it/wiki/Units . Please add only sensible ones
     */
    enum Unit
    {
        AML,
    ...
    //! Number of AmlBits (1e-8) per unit
    static qint64 factor(int unit);
    //! Number of AmlBits (1e-8) per unit for assets
    ...
    //! Return maximum number of base units (AmlBits)
    static CAmount maxMoney();

private:
    QList<AmlBitcoinUnits::Unit> unitlist;
};
typedef AmlBitcoinUnits::Unit AmlBitcoinUnit;

#endif // AMLBITCOIN_QT_AMLBITCOINUNITS_H
