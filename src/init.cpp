 ...
 * between AmlBitcoind, and AmlBitcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("AmlBitcoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateAmlBitcoins(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), AMLBITCOIN_CONF_FILENAME));
    if (mode == HMM_AMLBITCOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), AMLBITCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AmlBitcoinProject/AmlBitcoin>";
    const std::string URL_WEBSITE = "<https://AmlBitcoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("AmlBitcoin-loadblk");
...
/** Sanity checks
 *  Ensure that AmlBitcoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AmlBitcoin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single AmlBitcoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", AMLBITCOIN_CONF_FILENAME)).string());
    ...
    GenerateAmlBitcoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
