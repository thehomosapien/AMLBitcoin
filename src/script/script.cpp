...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** AML START */
    case OP_AML_ASSET              : return "OP_AML_ASSET";
    /** AML END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_AML_ASSET) { // OP_AML_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == AML_R) { // Check to see if AML starts at 27 ( this->size() < 105)
                if ((*this)[28] == AML_V)
                    if ((*this)[29] == AML_N)
                        index = 30;
            } else {
                if ((*this)[28] == AML_R) // Check to see if AML starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == AML_V)
                        if ((*this)[30] == AML_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == AML_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == AML_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == AML_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == AML_R) {
