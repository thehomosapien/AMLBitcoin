...
The RPM spec file provided here is for AmlBitcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://AmlBitcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[AmlBitcoin git repository](https://github.com/AmlBitcoinProject/AmlBitcoin) but are not part of
...
Currently only `Source10` is used, for the example `AmlBitcoin.conf` file.
...
in the AmlBitcoin git repository. At present only an SVG version of the AmlBitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the AmlBitcoin
...
needed when 0.12.1 is released, a proper fix is already in the AmlBitcoin git
master branch.
...
idea to build AmlBitcoin Core against a static version of that library compiled
...
The `AmlBitcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `AmlBitcoin-server` package. At this point, most RPM based
...
The `AmlBitcoin-server` package should have SELinux support. How to properly do
...
AmlBitcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/AmlBitcoin/). He has been
packaging AmlBitcoin for Fedora at least since 2012.
...
4. I renamed the `AmlBitcoin` package that contains the Qt GUI to `AmlBitcoin-core` as
that appears to be how the general population refers to it, in contrast to
`AmlBitcoin-xt` or `AmlBitcoin-classic`. I wanted to make sure the general population
