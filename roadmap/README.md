...
AmlBitcoin (AML) is a Proof of Work coin built on the Bitcoin UTXO model. As with other Bitcoin derivatives, AML coins are distributed to persons augmenting the AmlBitcoin network by mining AmlBitcoin.
...
ASIC Resistance - A published commitment to continual attempts at ASIC resistance. If ASICs are created for x16r, then we will, at a specific block number, modify one of the algorithms to add some varients of Equihash or similar efforts to increase the resistance to ASIC miners for AmlBitcoin.

#### Asset Support

AmlBitcoin will be a hard fork that extends AmlBitcoin to include the ability to issue and transfer assets. The expected release of asset capabilities will be approximately seven months after the release of AML. AmlBitcoin will be extended to allow issuing, reissuing, and transfer of assets. Assets can be reissuable or limited to a set supply at the point of issuance. The cost to create assets will be 500 AML to create any qty of an asset. Each asset name must be unique. Asset names will be limited to A-Z and 0-9, '_' and '.' and must be at least three characters long. The '.' and the '_' cannot be the first, or the last character, or be consecutive.  
...
The AML used to issue assets will be sent to a burn address, which will reduce the amount of AML available.  

Asset transfers require the standard AML transaction fees for transfer from one address to another.
...
Reward capabilities will be added to allow payment (in AML) to all holders of an asset. Payments of AML would be distributed to all asset holders pro rata. This is useful for paying dividends, dividing payments, or rewarding a group of token holders.  

Example: A small software company issues an asset GAMECO that represents a share of the project. GAMECO tokens can be traded with others. Once the software company profits, those profits can be distributed to all holders of GAMECO by sending the profits (via AML) to all holders of GAMECO.

#### Block Size

AmlBitcoin may increase the blocksize from 2 MB to X MB to allow for more on-chain transactions.

### Phase 3 - Rewards

Rewards allow payment in AML to asset holders.

[More on rewards...](./rewards/README.md)

### Phase 4 - Unique Assets (Complete)

Once created, assets can be made unique for a cost of 5 AML. Only non-divisible assets can be made unique. This moves an asset to a UTXO and associates a unique identifier with the txid. From this point the asset can be moved from one address to another and can be traced back to its origin. Only the issuer of the original asset can make an asset unique.  
...
Messaging to token holders by authorized senders will be layered on top of the Phase 4 unique assets. See [KAAAWWW Protocol](https://medium.com/@tronblack/AmlBitcoin-kaaawww-2f72077aece) for additional information.
...
Voting will be accomplished by creating and distributing parallel tokens to token holders. These tokens can be sent to AML addresses to record a vote.

[More on voting...](./voting/README.md)

### Phase 7 - Compatibility Mode

Allows newly created assets to appear exactly like AML, LTC, or Bitcoin for easy integration into exchanges, wallets, explorers, etc.
...
Issue an asset with unique name. Unit as 1 for whole units, or 0.00000001 for AmlBit-like units. Qty should be whole number. Reissuable is true/false for whether additional units can be issued by the
original issuer.  
